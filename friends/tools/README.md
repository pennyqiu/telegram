...
- `fetch_us_dip.py`：`period="max"` 全历史取 ATH，用**盘中最高价**；`auto_adjust=False` 不复权。回撤 = (现价 − ATH) / ATH，按 watchlist 阶梯给"观望 / 分批买入 / 已到底档"。
  ...
  - **每只标的的收盘日期会显式标出**，且核对是否恰好等于"昨天"（见 5.7）。不等于时邮件、页面都标红 `⚠慢1天`，但不拦截信号——数据仍是真实历史价，只是不是最新的。
...
**修法不是调低阈值**（调到 1 会在美股假日、或 yfinance 单只延迟时到处误报），而是
换一个更准的不变量：这套 cron 只在北京周二~周六早上跑、取"前一晚"美股收盘，在这个
排班下 **`last_date` 应该恰好等于「昨天」，从不跨周末**（周六的前一天是周五）。
`build_payload` 现在按这条精确比对，不等于就在邮件、页面里标红显示（`⚠慢1天`），
但**不拦截信号**——数据仍是真实历史价，只是不是最新的，该不该按它动手交给人判断，
不该像 5.6 节那样直接决定"要不要显示"。同时 `fetch_quotes` 会在日志里记录这一轮
yfinance 成功几只、东财兜底几只，全部失败转兜底时额外打一条 `[warn]`。
