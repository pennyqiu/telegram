...
1. 分红实现率是什么 — 监管文本与会计语言的对照
2. 实现率背后的「3 池模型」：现金红利 / 复归红利 / 终期红利
3. 统计学视角：用方差与中位数看「实现率的稳定性」
4. 新保单实现率为何普遍 100%+ — 一个公开的精算秘密
5. 15 家公司近 5 年实现率横评（脱敏数据）
6. 主动暴露劣势：实现率指标本身的 4 个局限
7. 实操：投保前必须问代理的 6 个问题
...
> ⚠️ **劣势暴露：** **劣势暴露 #1：**很多公司在公告里只披露「现金红利实现率」，故意不披露「终期红利实现率」。这是合规边缘的取巧 — 因为终期红利在你退保之前都不会真正派发，所以技术上「无从计算实现率」。但终期红利占总收益 50%–65%，**等于把最重要的指标藏起来了**。
...
> ⚠️ **劣势暴露：** **实操建议：**如果某公司主打「我们某新产品实现率 105%」，请要求代理同时披露「该公司同类老产品（>10 年）的实现率分布」。如果对方答不出来或闪烁其词，建议慎重。
