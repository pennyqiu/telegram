...
1. 为什么这篇文章对科技新贵特别重要
2. RSU / ESOP 在内地的税务全流程
3. 「港险递延税款」话术的真假拆解
4. 真正合法的税务规划路径（3 种）
5. 跨境身份与税务居民的灰色地带
6. 港险在 RSU 财富中的真正角色
7. 3 个真实案例的脱敏复盘
8. 清单：行权前 6 个月必须做的 8 件事
...
> 🚨 **严重警告：** **严重澄清：**「**用港险递延 RSU 税款**」是**错误的**陈述。RSU 的 IIT 是已经发生的纳税义务，不能通过任何金融工具「递延」。任何承诺可以做到这一点的代理，要么是不懂税法，要么是故意误导。
...
> ⚠️ **劣势暴露：** **关键提醒：**跨境身份是真正能合法降低 IIT 的路径，但需要：① 全家长期规划（至少 5-10 年）；② 真实居住而非纸面操作；③ 配偶 / 子女的连带安排；④ 资产 / 业务的全面跨境结构化。**这不是"买个保单就能解决"的事情**。
...
对资产 ≥ 1000 万美元的科技新贵，港险的「跨代结构」（参见 **CPA 研报 #06**（见姐妹篇））是真正有价值的功能。
...
> 「**用港险递延 RSU 税款**」是错误的销售话术。RSU 的 IIT 不能递延，只能优化（通过归属时点、跨境身份等合法路径）。
