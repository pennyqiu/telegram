...
| [期权议题调研提示词.md](./期权议题调研提示词.md) | 跑一条期权议题：策略调研 → 白名单裁决 → 规则条款建议。配合 [期权专题/09-知识底稿](./期权专题/09-知识底稿-给大模型.md) 使用 | 按需 |
...
上面四层都靠「到点了记得做」，而 [决策日志](./决策日志.md) 第 134 行的结论正是
「**首要矛盾是执行率，不是认知准确率**」——首次复盘发现的六条 playbook 规则全部未执行，
问题不在认知层面，在缺少定期比对的机制。

所以四层之下还垫了一个每天跑的脚本 [`tools/check_discipline.py`](./tools/README.md)。
它不产生任何新判断，只回答一个问题：**你自己写下的时限，今天有哪些到了。**

检查四件事，全部只依赖日期、不依赖行情：决策日志里逾期与临期的条目、期权 21 DTE
时间闸门（规则 A3 / T5）、到期日已过却还挂在台账上的头寸、距上次复盘多少天。
规则原文运行时从规则卡解析，不另抄一份。

```bash
python3 tools/check_discipline.py                    # 看今天
python3 tools/check_discipline.py --today 2026-09-20 # 看未来某天会报什么
```

它以决策日志的状态列为准。**处理完请顺手把状态改掉**，否则会一直被报出来——
这是刻意的，让台账不更新的成本立刻显现，而不是攒到下次复盘才发现对不上。
