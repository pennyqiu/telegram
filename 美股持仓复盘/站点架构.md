...
| 侧边栏分组名 / 排序 | `tools/build_site.py` 里 `GROUP_ORDER` / `CORE_ORDER` / `SYMBOL_ORDER` / `classify()` | 重跑脚本 |
...
| 标的档案 | `标的档案/` | `SYMBOL_ORDER`（按穿透仓位大致顺序） |
| 操作手册 | `playbooks/` | 文件名 |
...
2. `投资规则卡.md` — 能开多大、行权价是否合规  
3. `playbooks/<代码>-*.md` — 挂什么价、什么周期、展期/止损  
4. `决策日志.md` — 手册规则有没有执行  
